...
namespace Wikipedia_Seçim_Sonuçları
{
	public partial class Form1 : Form
	{
		...
		private async Task MetropolitanData()
		{
			...
			urlCandidate = "https://acikveri.ysk.gov.tr/api/getBuyuksehirBelediyeBaskanligiAdayListesi?secimId=20260&ilId=1";
			...
			{
				richTextBox1.AppendText($"{{{{Siyasi parti haritarenk|{x.Party}|{textInfo.ToTitleCase(x.Candidate)}|{x.Votes.ToString("N0")}|%{x.Percentage.ToString().Replace(",", ".")}{(count == 0 ? "|1" : "|0")}{(x.Party != x.PartyFullName ? "|" + x.PartyFullName + "}}" : "}}")}\n");
				count++;
			});
		}
		private async Task Province()
		{
			for (int i = 1; i < 82; i++)
			{
				...
				foreach (var province in provinceData)
				{
					...
					mergedDatas.ForEach(x =>
					{
						richTextBox2.AppendText($"{{{{Siyasi parti haritarenk|{x.Party}|{textInfo.ToTitleCase(x.Candidate)}|{x.Votes.ToString("N0")}|%{x.Percentage.ToString().Replace(",", ".")}{(count == 0 ? "|1" : "|0")}{(x.Party != x.PartyFullName ? "|" + x.PartyFullName + "}}" : "}}")}\n");
						count++;
					});
